...
class PlanProteicoController extends Controller
{
    public function index($id)
    {
        ...
        $municipios->each(function ($municipio) {
            $ferias = Proteico::where('fecha', 'LIKE', "%".date('Y')."%")->where('municipios_id', $municipio->id)->get();
            $municipio->ferias = $ferias->count();
            $municipio->familias = $ferias->sum('familias');
            $municipio->tm = $ferias->sum('tm');
        });

        $ferias = Proteico::where('fecha', 'LIKE', "%".date('Y')."%")->get();
    ...
    public function verMunicipio($id, $id_municipio)
    {
        ...
        $parroquias->each(function ($parroquia) {
            $ferias = Proteico::where('fecha', 'LIKE', "%".date('Y')."%")->where('parroquias_id', $parroquia->id)->get();
        ...
        $ferias = Proteico::where('fecha', 'LIKE', "%".date('Y')."%")->where('municipios_id', $id_municipio)->get();
        $total_ferias = $ferias->count();
        $total_familias = $ferias->sum('familias');
        $total_tm = $ferias->sum('tm');


        return view('android.plan_proteico.municipio')
    ...
    public function verParroquia($id, $id_municipio, $id_parroquia)
    {
        ...
        $ferias = Proteico::where('fecha', 'LIKE', "%".date('Y')."%")->where('parroquias_id', $id_parroquia)->orderBy('fecha', 'ASC')->get();
        $total_ferias = $ferias->count();
        $total_familias = $ferias->sum('familias');
        $total_tm = $ferias->sum('tm');

        return view('android.plan_proteico.parroquia')
            ->with('municipio', $municipio)
