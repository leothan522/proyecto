...
class TiendaMovilController extends Controller
{
    public function index($id)
    {
        ...
        $municipios->each(function ($municipio) {
            $ferias = Movil::where('fecha', 'LIKE', "%".date('Y')."%")->where('municipios_id', $municipio->id)->get();
            $municipio->ferias = $ferias->count();
            $municipio->familias = $ferias->sum('familias');
            $municipio->tm = $ferias->sum('tm');
        });

        $ferias = Movil::where('fecha', 'LIKE', "%".date('Y')."%")->get();
    ...
    public function verMunicipio($id, $id_municipio)
    {
        ...
        return view('android.tienda_movil.municipio')
    ...
    public function verParroquia($id, $id_municipio, $id_parroquia)
    {
        ...
        $ferias = Movil::where('fecha', 'LIKE', "%".date('Y')."%")->where('parroquias_id', $id_parroquia)->orderBy('fecha', 'ASC')->get();
        $total_ferias = $ferias->count();
        $total_familias = $ferias->sum('familias');
        $total_tm = $ferias->sum('tm');

        return view('android.tienda_movil.parroquia')
            ->with('municipio', $municipio)
