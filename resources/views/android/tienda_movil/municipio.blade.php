                    ...
                    <ul class="nav nav-pills flex-column">
                        @foreach ($parroquias as $parroquia)
                            <li class="nav-item active">
                                <a href="{{ route('android.tienda_movil_parroquia', [Auth::user()->id, $municipio->id, $parroquia->id]) }}" class="nav-link" onclick="verCargando()">
                                    <span class="text-sm">{{ $parroquia->nombre_completo }}
                                    <span class="float-right justify-content-center row col-8">
                                        <span class="badge bg-primary col-3">{{ formatoMillares($parroquia->ferias, 0) }}</span>
                                        <span class="col-1"></span>
										<span class="badge bg-purple col-3">{{ formatoMillares($parroquia->familias, 0) }}</span>
                                        <span class="col-1"></span>
                                        <span class="badge bg-pink col-3">{{ formatoMillares($parroquia->tm, 2) }}</span>
                                    </span>
                                    </span>
                                </a>
                            </li>
